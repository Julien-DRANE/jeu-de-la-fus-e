...
function generateObstacles(count = 1) {
    for (let i = 0; i < count; i++) {
        ...
        let speed = (Math.random() * 2 + 1.5) * scaleFactor; // Ajusté pour être légèrement moins rapide
...
function increaseDifficulty() {
    if (currentLevel === 1 || currentLevel === 2) {
        ...
        if (currentLevel === 1) {
            obstacleSpawnInterval = Math.max(500, obstacleSpawnInterval - 45); // Diminuer l'intervalle à minimum 500 ms
        } else if (currentLevel === 2) {
